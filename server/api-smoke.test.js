...
test('family flow stays isolated, authorized and internally consistent', async () => {
  ...
  assert.equal(health.body.version, '1.11.0');
  const appRelease = await request('/api/app/version');
  assert.equal(appRelease.body.versionName, '1.11.0');
  assert.equal(appRelease.body.versionCode, 26);
  ...
  assert.equal(bootstrap.body.appVersion, '1.11.0');
  assert.equal(bootstrap.body.releaseNotes.version, '1.11.0');
  ...
  assert.equal(acknowledgedReleaseNotes.body.version, '1.11.0');
  assert.equal(
  ...
  assert.equal(secondAdultBootstrap.body.releaseNotes.version, '1.11.0');
  await request('/api/auth/member', {
    method: 'POST',
    headers: authenticatedHeaders,
  ...
  assert.equal(problemReport.body.report.appVersion, '1.11.0');
  await request(
    '/api/problem-reports',
    { headers: authenticatedHeaders },
    403
